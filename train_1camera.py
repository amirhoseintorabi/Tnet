...
if __name__ == "__main__":
    ...
    #model = tnet.tnet_mount("InceptionResNetV2")
    plot=False
    #plot=True
    ...
    [above,mount1,_,dataset,Y]=pickle.load(open('Stereo'+'.pickle', 'rb'))
    ...
    if(Auxiliary_loss==False):
        loss_functions={model_name+'_pose_xy': tnet.euc_loss3x, model_name+'_pose_r': tnet.euc_loss3r,}
    else:
        loss_functions={model_name+'_pose_xy_a1': euc_loss1x, model_name+'_pose_r_a1': euc_loss1r,
                        model_name+'_pose_xy_a2': euc_loss2x, model_name+'_pose_r_a2': euc_loss2r,
    ...
    LRReduce = ReduceLROnPlateau(monitor='val_loss', factor=0.1   ,patience=4, min_lr=1e-11,verbose=1)
    ...
    #create model
    model = tnet.TNet_1Camera(model_name,Tune,Auxiliary_loss,BlockFirsts)     
    ...
    try:
        history = model.fit(train_input,train_out, batch_size=batch_size, epochs=epoch_num, validation_data=(test_input,test_output), callbacks=[early_stopping, checkpointer,LRReduce,LRScheduler,csv_logger])
    except Exception as e:
        print("Oops!", e.__class__, "occurred.")
